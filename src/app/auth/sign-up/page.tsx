...
import { firebaseAuth, googleSignIn } from "@/lib/firebaseClient";
...
export default function SignUpPage() {
  ...
  async function oauth(provider: "google" | "microsoft" | "apple") {
    try {
      ...
      if (provider === "google") {
        // Uses your working Google flow + /api/auth/session
        await googleSignIn();
        router.replace("/admin"); // or "/account" depending on your app
        return;
      }

      alert(`${provider} sign-up not configured`);
    } catch (e: unknown) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!r.ok) {
        // Your backend currently rejects non-admins. Show message and clean up.
        await signOut(firebaseAuth);

        const data = await r.json().catch(() => ({}));
        throw new Error(
          data?.error ||
            "No account found / registration not allowed. Please contact support."
        );
      }

      router.replace("/admin"); // or "/account"
    } catch (e: unknown) {
      const message = e instanceof Error ? e.message : "Sign-up failed";
      alert(message);
    } finally {
