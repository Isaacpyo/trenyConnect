...
export default function SignInPage() {
  ...
  async function oauth(provider: "google" | "microsoft" | "apple") {
    try {
      setLoading(true);
      if (provider === "google") {
        await googleSignIn();          // <-- real Google sign-in
        router.replace(next);          // redirect after cookie is set
      } else {
        // TODO: add Microsoft & Apple providers later
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      // TODO: implement email/password sign-in if you need it
...
}: { provider: "google" | "microsoft" | "apple"; onClick: () => void; disabled?: boolean }) {
  const map = {
